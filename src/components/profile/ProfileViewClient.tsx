  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-ink-muted/70">{email}</p>
              {profile.bio && (
                <p className="mt-4 max-w-xl text-sm leading-relaxed text-ink-muted">{profile.bio}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="grid w-full max-w-md grid-cols-2 gap-3 sm:grid-cols-3 lg:w-auto">
              <TrustTile label={t("profile.trustScore")} value={profile.rating.toFixed(2)} hint={t("profile.rollingDays")} />
              <TrustTile label={t("profile.exchanges")} value={String(profile.exchanges)} hint={t("profile.completedNearby")} />
              <TrustTile label={t("profile.response")} value={t("profile.responseFast")} hint={t("profile.responseHint")} className="col-span-2 sm:col-span-1" />
            </div>
            <Link
              href="/profile/edit"
              className="inline-flex items-center justify-center rounded-full bg-brand px-6 py-2.5 text-sm font-semibold text-white shadow-brand-soft-sm transition hover:bg-brand-dim"
            >
      ...
      <section className="mt-10">
        <h2 className="font-display text-lg font-semibold text-ink">{t("profile.activeListings")}</h2>
              ...
              >
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                    {l.images?.[0] ? (
                      <Image src={l.images[0]} alt={l.title} fill className="object-cover transition group-hover:scale-105" sizes="(max-width:640px)100vw,(max-width:1024px)50vw,33vw" />
                    ) : (
                      <div className="flex h-full items-center justify-center text-3xl text-slate-300 dark:text-slate-600">📦</div>
                    )}
                    <span className={cn(
                      "absolute right-2 top-2 rounded-full px-2.5 py-0.5 text-[11px] font-bold",
                      l.status === "available" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" : "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400"
                    )}>
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl border border-black/[0.06] bg-white/60 p-5 dark:border-white/10 dark:bg-slate-900/50">
            ...
            <ul className="mt-3 space-y-2 text-xs text-ink-muted">
              <li className="flex gap-2"><span className="text-brand">●</span> {t("profile.csPublicMeetups")}</li>
              <li className="flex gap-2"><span className="text-brand">●</span> {t("profile.csAccurateCondition")}</li>
              <li className="flex gap-2"><span className="text-brand">●</span> {t("profile.csRespectfulCancel")}</li>
            </ul>
...
function TrustTile({ label, value, hint, className }: { label: string; value: string; hint: string; className?: string }) {
  return (
    <div className={cn(
